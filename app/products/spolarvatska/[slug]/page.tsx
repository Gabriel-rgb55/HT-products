...
export default async function WasherFluidProductPage({ params }: ProductPageProps) {
  const { slug } = await params;
  const product = washerFluids.find((washerFluid) => washerFluid.slug === slug);
    ...
    <main className="min-h-screen bg-[#050816]">
      <section className="border-b border-white/10">
        <div className="mx-auto max-w-7xl px-6 py-5">
          <div className="flex flex-wrap items-center gap-2 text-sm text-slate-500">
            ...
            <Link href="/products/spolarvatska" className="transition hover:text-white">Spolarvätska</Link><span>/</span>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 lg:py-20">
        <div className="grid gap-12 lg:grid-cols-2 lg:items-start">
          <div className="relative min-h-[500px] overflow-hidden rounded-3xl border border-white/10 bg-[#0a0e1a]">
            ...
            <div className="absolute left-5 top-5 rounded-full border border-white/10 bg-black/40 px-4 py-2 text-sm font-medium text-white backdrop-blur-md">{product.temperature}</div>
          </div>
          ...
          <div>
            ...
            <div className="mt-10 grid grid-cols-2 gap-4">
              <div className="rounded-2xl border border-white/10 bg-white/5 p-5"><p className="text-sm text-slate-500">Frostskydd</p><p className="mt-1 font-semibold text-white">{product.temperature}</p></div>
            ...
            <div className="mt-10 flex flex-col gap-4">
              <AddToQuoteButton slug={`spolarvatska-${product.slug}`} name={product.name} image={product.image} sizes={product.sizes} />
      ...
      <section className="mx-auto max-w-7xl px-6 py-20">
        <div className="rounded-3xl border border-blue-500/20 bg-blue-500/5 p-8 md:flex md:items-center md:justify-between md:gap-10 lg:p-10">
          ...
          <div className="mt-6 md:mt-0"><AddToQuoteButton slug={`spolarvatska-${product.slug}`} name={product.name} image={product.image} sizes={product.sizes} /></div>
        </div>
      </section>
    </main>
  );
