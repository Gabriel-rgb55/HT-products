    ...
    <main className="min-h-screen bg-[#050816]">
      <section className="border-b border-white/10">
        <div className="mx-auto max-w-7xl px-6 py-5">
          <div className="flex items-center gap-2 text-sm text-slate-500">
            ...
            <span className="text-slate-300">Kylarvätska</span>
          </div>
        </div>
      </section>
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-16 lg:py-20">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-blue-400">MOTOREX Kylsystem</p>
            <h1 className="mt-4 text-5xl font-bold text-white md:text-6xl">Kylarvätska</h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-400">
              Utforska kylarvätskor för effektiv värmeöverföring och långvarigt skydd mot frost, korrosion, kavitation och överhettning.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              {["G11", "G12+", "G13", "Heavy Duty"].map((tag) => (
      ...
      <section className="mx-auto max-w-7xl px-6 py-16">
        <div className="flex flex-col gap-6 border-b border-white/10 pb-8 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-bold text-white">Våra kylarvätskor</h2>
            <p className="mt-2 text-slate-500">{coolants.length} produkter visas</p>
          </div>
          <div className="flex flex-wrap gap-3">
            <button className="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-medium text-white">Alla</button>
            {["Personbil", "Yrkesfordon", "Maskiner"].map((filter) => (
          ...
          {coolants.map((product) => (
            <article key={product.id} className="group flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0a0e1a] transition duration-300 hover:-translate-y-1 hover:border-blue-500/60">
              <div className="relative h-72 overflow-hidden bg-[#0d1220]">
                ...
                <div className="absolute left-4 top-4 rounded-full border border-white/10 bg-black/40 px-3 py-1.5 text-xs font-medium text-white backdrop-blur-md">{product.standard}</div>
              </div>
              <div className="flex flex-1 flex-col p-6">
                ...
                <div className="mt-auto pt-6">
                  <Link href={`/products/kylarvatska/${product.slug}`} className="flex items-center justify-between border-t border-white/10 pt-5 text-sm font-medium text-blue-400 transition hover:text-blue-300">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-20">
        <div className="rounded-3xl border border-blue-500/20 bg-blue-500/5 p-8 md:flex md:items-center md:justify-between md:gap-10">
          <div>
            <h2 className="text-2xl font-semibold text-white">Osäker på vilken kylarvätska du behöver?</h2>
            <p className="mt-2 max-w-2xl leading-7 text-slate-400">Rätt kylarvätska beror på fordon, material i kylsystemet och tillverkarens specifikation. Kontakta oss så hjälper vi dig att välja rätt.</p>
          </div>
